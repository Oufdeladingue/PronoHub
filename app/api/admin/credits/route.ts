...
export async function GET(request: NextRequest) {
  try {
    ...
    // Requête pour compter le total d'utilisateurs (inclut tous les utilisateurs, y compris super_admin)
    ...
    // Requête pour récupérer les utilisateurs avec pagination (inclut tous les utilisateurs)
    ...
    const userStats = await Promise.all(
      (usersData || []).map(async (user) => {
        ...
        // Récupérer les slots disponibles (non utilisés)
