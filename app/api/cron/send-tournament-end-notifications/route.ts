      ...
      for (const participant of participants) {
        ...
        if (profile.fcm_token) {
          ...
          if (!pushLog || pushLog.length === 0) {
            ...
            const title = loc === 'en' ? config.defaultTitleEn : loc === 'es' ? config.defaultTitleEs : loc === 'de' ? config.defaultTitleDe : loc === 'it' ? config.defaultTitleIt : config.defaultTitle
            const body = (loc === 'en' ? config.defaultBodyEn : loc === 'es' ? config.defaultBodyEs : loc === 'de' ? config.defaultBodyDe : loc === 'it' ? config.defaultBodyIt : config.defaultBody).replace('{tournamentName}', tournament.name)
