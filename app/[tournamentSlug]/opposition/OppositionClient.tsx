...
export default function OppositionClient({
  ...
  const [showFinishedMatches, setShowFinishedMatches] = useState(false) // Afficher/masquer les matchs terminés de la journée
  ...
  useEffect(() => {
    if (selectedMatchday !== null && tournament) {
      ...
      setShowFinishedMatches(false) // Replier les matchs terminés à chaque changement de journée

      // OPTIMISÉ: Filtrage en mémoire (instantané) puis requêtes DB en parallèle
      const loadData = async () => {
        try {
          // Étape 1: Filtrer les matchs en mémoire (0ms, pas de requête DB)
          const filteredMatches = filterMatchesForMatchday(selectedMatchday)
          setMatches(filteredMatches)

          // Initialiser les prédictions à 0-0 pour les matchs sans prédiction
          ...
          // Étape 2: Requêtes DB en parallèle avec les match IDs déjà connus
          const matchIds = filteredMatches.map(m => m.id)
          await Promise.all([
            fetchUserPredictions(matchIds),
  ...
  // OPTIMISÉ: Filtrage en mémoire au lieu de requêtes DB (allMatches déjà chargé depuis le server)
  const filterMatchesForMatchday = (matchday: number): Match[] => {
  ...
  // OPTIMISÉ: Accepte les matchIds directement au lieu de faire des requêtes waterfall
  const fetchUserPredictions = async (matchIds: string[]) => {
    try {
  ...
  const fetchMatchPoints = async (filteredMatches: Match[]) => {
    ...
    try {
      ...
      // Filtrer les matchs avec scores (terminés ou en cours) directement depuis les données en mémoire
  ...
  // Repli des matchs terminés : par défaut on n'affiche que les matchs non terminés
  // (les pronostiquables remontent en haut), avec une barre pour réafficher les terminés.
  const finishedMatchesList = matches.filter(isMatchFinished)
  const activeMatchesList = matches.filter((m) => !isMatchFinished(m))
  const canCollapseFinished = finishedMatchesList.length > 0 && activeMatchesList.length > 0
  const displayedMatches = !canCollapseFinished || showFinishedMatches ? matches : activeMatchesList
                  ...
                  <div className="space-y-6">
                    ...
                    {canCollapseFinished && (
                      <button
                        ...
                        className="mx-auto w-fit mb-4 px-4 py-2 rounded-lg flex items-center justify-center gap-2 text-sm font-semibold theme-text-secondary info-bg-container border theme-border hover:opacity-80 transition"
                      >
                        {showFinishedMatches ? (
                          // Œil ouvert : les matchs terminés sont affichés
                          <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" />
                            <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                          </svg>
                        ) : (
                          // Œil barré : les matchs terminés sont masqués
                          <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                ...
                                <div className={`md:hidden relative ${isClosed ? 'opacity-75' : ''}`}>
                                  ...
                                  <div className="grid grid-cols-3 gap-2 mb-3">
                                    ...
                                    <div className="flex flex-col items-center gap-1">
                                      {/* Placeholder pour aligner avec le badge "Défaut" de la colonne adverse */}
                                ...
                                {isClosed && (
                                  <div className="mt-2 border-t theme-border pt-2">
                                    ...
                                    >
                                      ...
                                      <span className={expandedMatches.has(match.id) ? '' : 'effect-shine'}>Voir les pronostics de mes adversaires</span>
                                    </button>
          ...
          {activeTab === 'regles' && (
            <div className="theme-card">
              ...
              <div className="space-y-4 theme-text-secondary">
                ...
                {/* Règle 90 minutes pour les compétitions avec phases éliminatoires */}
                {Object.values(matchdayStages).some(s => s && isKnockoutStage(s)) && (
                  <div className="quota-warning-box rounded-lg p-4">
                    <div className="flex items-start gap-2">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 quota-warning-icon flex-shrink-0 mt-0.5" viewBox="0 0 20 20" fill="currentColor">
                ...
                {tournament?.bonus_qualified && (
                  <div className="quota-warning-box rounded-lg p-4">
                    <div className="flex items-start gap-2">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-[#ff9900] flex-shrink-0 mt-0.5" viewBox="0 0 20 20" fill="currentColor">
