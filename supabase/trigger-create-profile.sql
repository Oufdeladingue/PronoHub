...
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.profiles (id, username, email, notification_preferences, has_chosen_username)
  VALUES (
    NEW.id,
    ...
    -- Toujours false : forcer le passage par l'écran de choix de pseudo pour tous
    false
  );
  RETURN NEW;
END;
