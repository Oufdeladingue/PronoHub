-- =====================================================
-- FIX: Compter les tournois PREMIUM par CRÉATION (pas participation)
-- GRATUIT: par participation (inchangé)
-- PREMIUM: par création (original_creator_id avec fallback creator_id)
-- =====================================================
...
SELECT
  ...
  -- Compteur tournois premium actifs (CRÉATION avec fallback, max 5)
  -- On utilise original_creator_id en priorité, avec fallback sur creator_id pour les anciens tournois
  (
    SELECT COUNT(DISTINCT t.id)
    FROM public.tournaments t
    WHERE t.tournament_type = 'premium'
    AND t.status != 'completed'
    AND (
      t.original_creator_id = p.id
      OR (t.creator_id = p.id AND t.original_creator_id IS NULL)
    )
  ...
  CASE
    -- A un abonnement actif avec des slots dispo (CRÉATION)
    WHEN us.status = 'active' AND (
      SELECT COUNT(DISTINCT t.id) FROM public.tournaments t
      WHERE t.tournament_type = 'premium' AND t.status != 'completed'
      AND (t.original_creator_id = p.id OR (t.creator_id = p.id AND t.original_creator_id IS NULL))
    ) < 5 THEN TRUE
    ...
    -- A encore des slots gratuits (PARTICIPATION)
    WHEN (
...
BEGIN
  ...
  -- Compter tournois premium actifs (CRÉATION avec fallback)
  SELECT COUNT(DISTINCT t.id) INTO v_premium_count
  FROM public.tournaments t
  WHERE t.tournament_type = 'premium'
  AND t.status != 'completed'
  AND (
    t.original_creator_id = p_user_id
    OR (t.creator_id = p_user_id AND t.original_creator_id IS NULL)
  );
