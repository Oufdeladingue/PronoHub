...
-- Récupérer l'ID de l'utilisateur Rom's
DO $$
DECLARE
  roms_user_id UUID;
...
BEGIN
  ...
  RAISE NOTICE 'Création des pronostics par défaut terminée';
END $$;
...
SELECT
  ...
  SUM(CASE WHEN pr.is_default_prediction = FALSE THEN 1 ELSE 0 END) as real_predictions
FROM predictions pr
JOIN profiles p ON p.id = pr.user_id
JOIN tournaments t ON t.id = pr.tournament_id
JOIN imported_matches im ON im.id = pr.match_id
WHERE p.username = 'Roms'
