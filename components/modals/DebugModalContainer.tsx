...
import { subscribeToDebugModals, DebugModalType, initDebugModals } from '@/lib/debug-modals'
import { usePurchaseModal } from '@/lib/hooks/use-purchase-modal'
import { X } from 'lucide-react'
import Image from 'next/image'

export default function DebugModalContainer() {
  const [activeModal, setActiveModal] = useState<DebugModalType | null>(null)
  const [tournamentId, setTournamentId] = useState<string | undefined>()
  const { handlePurchase, loading } = usePurchaseModal()

  useEffect(() => {
    initDebugModals()
    const unsubscribe = subscribeToDebugModals((state) => {
      setActiveModal(state.isOpen ? state.type : null)
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[9999] p-4">
      <div className="relative max-w-sm w-full rounded-2xl overflow-hidden shadow-2xl border border-[#ff9900]/25">
        ...
        {/* Badge DEBUG */}
        <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#ff9900] text-white text-xs font-bold px-3 py-1 rounded-full z-10">
          MODE DEBUG
        </div>
        ...
        <button
          onClick={handleClose}
          className="absolute top-2.5 right-2.5 z-10 w-8 h-8 flex items-center justify-center rounded-full bg-black/20 hover:bg-black/35 text-white/90 transition-all border border-white/10"
        ...
        >
          ...
          {activeModal === 'duration_extension' && (
            <>
              ...
              <div className="relative">
                <div
                  ...
                  style={{
                    inset: '-25px',
                    background: `
                      radial-gradient(ellipse 90% 100% at 50% 50%, rgba(0,0,0,.38) 25%, rgba(0,0,0,0) 70%),
                      radial-gradient(circle at 40% 50%, rgba(0,0,0,.25) 15%, rgba(0,0,0,0) 55%),
                      radial-gradient(circle at 60% 50%, rgba(0,0,0,.25) 15%, rgba(0,0,0,0) 55%)
                    `,
                    filter: 'blur(18px)'
                  }}
                />
          ...
          {activeModal === 'player_extension_2_1' && (
            <>
              <h2 className="text-white text-[26px] font-black leading-tight uppercase mb-0"
                  style={{ textShadow: '0 10px 26px rgba(0,0,0,.55)' }}>
                <span className="text-[#ffb000]" style={{ textShadow: '0 0 18px rgba(255,176,0,.22)' }}>
          ...
          {activeModal === 'player_extension_0' && (
            <>
              <h2 className="text-white text-[26px] font-black leading-tight uppercase mb-0"
                  style={{ textShadow: '0 10px 26px rgba(0,0,0,.55)' }}>
                <span className="text-[#ffb000]" style={{ textShadow: '0 0 18px rgba(255,176,0,.22)' }}>
