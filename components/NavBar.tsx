...
export default function NavBar({
  ...
  if (context === 'admin') {
    ...
    return (
      <div className="bg-gradient-to-r from-purple-600 to-purple-700 text-white shadow-lg sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4">
          {/* Badge Super Admin - masqué sur mobile */}
          <div className="hidden sm:block py-3 border-b border-purple-500">
          ...
          {/* Menu de navigation - scroll horizontal sur mobile */}
          <nav className="flex space-x-2 py-3 overflow-x-auto scrollbar-hide">
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`flex items-center gap-2 px-4 sm:px-5 py-2.5 rounded-lg transition-all duration-200 whitespace-nowrap flex-shrink-0 ${
                    isActive
    ...
    <nav className="theme-nav theme-nav-header hidden md:block">
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-3 gap-4 items-center">
          {/* COLONNE GAUCHE (33%) - Logo + Theme Toggle */}
          <div className="flex items-center gap-4">
            <Link href="/dashboard" className="nav-logo-wrapper">
              <img
                src="/images/logo.svg"
                alt="PronoHub"
                className="w-14 h-14 cursor-pointer hover:opacity-80 transition"
